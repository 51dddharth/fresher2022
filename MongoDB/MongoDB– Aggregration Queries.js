...
GitHUB Link: https://github.com/51dddharth/fresher2022.git
...
db.zipcodes.aggregate( [ { $group: { _id: { state: "$state", city: "$city", zipcode: "_id"}, zip: { $sum: 1 } } },{ $group: {  _id: "$_id.city", zcount: {$sum: "$zip"}} },{$sort: {zcount: -1} }] )

4.use $group to find the total population in Atlanta.
db.zipcodes.aggregate( [ { $group: { _id: { state: "$state", city: "$city", z:"_id"}, total: { $sum: 1 } } },{ $group: {  _id: {state:"$_id.state", city:"$_id.city"}, count: {$sum: "$total"}} } ])
...
db.zipcodes.aggregate([{ $sort : { population : -1, state: 1 } } ])

3.limit the results to just the first 3 results. What are the top 3 states in population?
db.zipcodes.aggregate([{ $group:{_id: {state:"$state",city: "$city" }  ,count: {$sum:1},population:{$sum:"$pop"}}},{$sort:{count:-1}}])
